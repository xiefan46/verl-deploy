...
#
# 策略：
#   首次安装：在 Container Disk (/opt/verl_env) 安装 conda 环境 → 压缩到 Network Volume (/workspace/)
...
log "[4/6] 安装 verl..."
cd "$VERL_ROOT"
installed verl && log "  verl 已安装，跳过" || pip install --no-deps -e .
...
if [ -d "/workspace" ]; then
    ...
    warn "/workspace 不存在（非 RunPod 环境），跳过缓存"
fi
